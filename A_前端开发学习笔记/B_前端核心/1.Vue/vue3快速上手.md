...
- 解决方案：
  ...
  - 使用`this.$set`或者`Vue.set`来给对象添加属性
  - 使用`this.$delete`或者`Vue.delete`来给对象删除属性
  - 使用重写后的7个方法操作数组
...
> Object.defineProperty()重复追加属性的时候，会报错
>
> Reflect.defineProperty()有返回值，虽然不能重复追加属性，但是不会报错，会返回false，用于封装框架的时候，框架会更加健壮
